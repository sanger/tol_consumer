...
class DataResolverStateMachine(StateMachine):
    ...
    request_resolution = valid.to(resolving) | resolved.to(resolved) | invalid.to(invalid)
    resolution_successful = resolving.to(resolved) | resolved.to(resolved) | invalid.to(invalid)
...
class DataResolverStateMachine2(StateMachine):
    # states
    pending = State("pending", initial=True)
    validating = State("validating")
    valid = State("valid")

...
class TestErrorCode:
    def test_validates_to_true(self):
    ...
    def test_can_convert_to_str(self):
        instance = build_instance()
    ...
    def test_message_for_trigger(self):
        instance = build_instance()
    ...
    def test_trigger_default_behaviour(self):
        instance = build_instance()
    ...
    def test_trigger_replicates_original_copy(self):
        instance = build_instance()
    ...
    def test_trigger_can_setup_origin_and_field(self):
        instance = build_instance()
    ...
    def test_trigger_logging_can_log(self):
    ...
    def test_trigger_logging_can_raise(self):
        instance = ErrorCode(
            "my type id", "root", "my field", "my description", level=LEVEL_ERROR, handler=HANDLER_RAISE
        )
        with patch("tol_lab_share.error_codes.logger.error") as logger:
        ...
        instance = ErrorCode(
            "my type id", "root", "my field", "my description", level=LEVEL_FATAL, handler=HANDLER_RAISE
        )
        with patch("tol_lab_share.error_codes.logger.fatal") as logger:
            with pytest.raises(ExceptionErrorCode):
                instance.trigger()
